...
export function CardStack<T extends CardStackItem>({
  ...
  // Override card dimensions on small screens
  const responsiveWidth = winWidth > 0 && winWidth < 640
    ? Math.min(cardWidth, winWidth - 80)
    : winWidth > 0 && winWidth < 768
    ? Math.min(cardWidth, winWidth - 120)
    : cardWidth;
  const responsiveHeight = winWidth > 0 && winWidth < 640
    ? Math.round(responsiveWidth * 0.65)
    : cardHeight;

  // Use responsive values
  cardWidth = responsiveWidth;
  cardHeight = responsiveHeight;
  ...
  const effectiveOverlap = winWidth > 0 && winWidth < 640 ? Math.min(overlap, 0.35) : overlap;
